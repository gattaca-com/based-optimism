...
func RequireL1Heads(t helpers.Testing, system *InteropDSL, latest, finalized uint64) {
	safeHeader, err := system.Actors.L1Miner.EthClient().BlockByNumber(t.Ctx(), big.NewInt(int64(rpc.LatestBlockNumber)))
	require.NoError(t, err)
	require.Equal(t, latest, safeHeader.Number().Uint64(), "L1 latest number does not match expectation")
...
func (a *ChainSyncStatusAsserter) baseSyncStatusAssertions(nextSyncStatus *eth.SyncStatus) bool {
	result := true
	result = result && assert.True(a.t, AssertAncestorDescentantRelationship(a.t, a.chain, nextSyncStatus.CrossUnsafeL2.ID(), nextSyncStatus.UnsafeL2.ID()), "cross unsafe block id is not an ancestor of the next unsafe block id")
	result = result && assert.True(a.t, AssertAncestorDescentantRelationship(a.t, a.chain, nextSyncStatus.LocalSafeL2.ID(), nextSyncStatus.UnsafeL2.ID()), "local safe block id is not an ancestor of the next unsafe block id")
	result = result && assert.True(a.t, AssertAncestorDescentantRelationship(a.t, a.chain, nextSyncStatus.SafeL2.ID(), nextSyncStatus.UnsafeL2.ID()), "safe block id is not an ancestor of the next unsafe block id")
	result = result && assert.True(a.t, AssertAncestorDescentantRelationship(a.t, a.chain, nextSyncStatus.FinalizedL2.ID(), nextSyncStatus.UnsafeL2.ID()), "finalized block id is not an ancestor of the next unsafe block id")
	return result
}
...
type SyncStatusAssertion func(t require.TestingT, prevSyncStatus, nextSyncStatus *eth.SyncStatus) bool
...
func WithUnsafeAdvancesTo(expectedUnsafeNumber uint64) func(cfg *updateExpectedConfig) {
	return func(cfg *updateExpectedConfig) {
		cfg.unsafeBlockAssertion = func(t require.TestingT, prevSyncStatus, nextSyncStatus *eth.SyncStatus) bool {
...
func WithCrossUnsafeAdvancesTo(expectedCrossUnsafeNumber uint64) func(cfg *updateExpectedConfig) {
	return func(cfg *updateExpectedConfig) {
		cfg.crossUnsafeAssertion = func(t require.TestingT, prevSyncStatus, nextSyncStatus *eth.SyncStatus) bool {
...
func WithLocalSafeAdvancesTo(expectedLocalSafeNumber uint64) func(cfg *updateExpectedConfig) {
	return func(cfg *updateExpectedConfig) {
		cfg.localSafeAssertion = func(t require.TestingT, prevSyncStatus, nextSyncStatus *eth.SyncStatus) bool {
...
func WithSafeAdvancesTo(expectedSafeNumber uint64) func(cfg *updateExpectedConfig) {
	return func(cfg *updateExpectedConfig) {
		cfg.safeAssertion = func(t require.TestingT, prevSyncStatus, nextSyncStatus *eth.SyncStatus) bool {
...
func WithFinalizedAdvancesTo(expectedFinalizedNumber uint64) func(cfg *updateExpectedConfig) {
	return func(cfg *updateExpectedConfig) {
		cfg.finalizedAssertion = func(t require.TestingT, prevSyncStatus, nextSyncStatus *eth.SyncStatus) bool {
