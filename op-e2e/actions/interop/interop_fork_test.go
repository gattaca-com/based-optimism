...
func TestInteropUpgrade(gt *testing.T) {
	...
	for _, c := range chains {
		...
		}, dsl.WithLocalSafeAdvancesTo(1), dsl.WithSafeAdvancesTo(1)) // Assert unsafe head does not move, but update crossUnsafe, locaSafe and safe heads
...
func assertProgramOutputMatchesDerivationForBlockTimestamp(t *testing.T, system *dsl.InteropDSL, endTimestamp uint64) {
	...
	require.True(t, runFppAndChallengerTests(t, system, []*transitionTest{
		{
