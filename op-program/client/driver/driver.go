...
func NewDriver(logger log.Logger, cfg *rollup.Config, l1Source derive.L1Fetcher,
	...
	channels := engine.StartPreconf(context.Background(), l2Source)
	engineDeriv := engine.NewEngDeriver(logger, context.Background(), cfg, metrics.NoopMetrics, ec, channels)
