...
func TestDataFromEVMTransactions(t *testing.T) {
	...
	cfg := &rollup.Config{
		...
		Genesis: rollup.Genesis{
			SystemConfig: eth.SystemConfig{
				BatchInboxAddr: crypto.PubkeyToAddress(inboxPriv.PublicKey),
			},
		},
	}
	...
	testCases := []calldataTest{
		{
			name: "simple",
			txs:  []testTx{{to: &cfg.Genesis.SystemConfig.BatchInboxAddr, dataLen: 1234, author: batcherPriv, good: true}},
		},
		{
			name: "other inbox",
		...
		{
			name: "other author",
			txs:  []testTx{{to: &cfg.Genesis.SystemConfig.BatchInboxAddr, dataLen: 1234, author: altAuthor, good: false}}},
		{
			name: "inbox is author",
			txs:  []testTx{{to: &cfg.Genesis.SystemConfig.BatchInboxAddr, dataLen: 1234, author: inboxPriv, good: false}}},
		{
		...
		{
			name: "empty tx",
			txs:  []testTx{{to: &cfg.Genesis.SystemConfig.BatchInboxAddr, dataLen: 0, author: batcherPriv, good: true}}},
		{
			name: "value tx",
			txs:  []testTx{{to: &cfg.Genesis.SystemConfig.BatchInboxAddr, dataLen: 1234, value: 42, author: batcherPriv, good: true}}},
		{
		...
		},
		{
			name: "mixed txs",
			txs: []testTx{
				{to: &cfg.Genesis.SystemConfig.BatchInboxAddr, dataLen: 1234, value: 42, author: batcherPriv, good: true},
				{to: &cfg.Genesis.SystemConfig.BatchInboxAddr, dataLen: 3333, value: 32, author: altAuthor, good: false},
				{to: &cfg.Genesis.SystemConfig.BatchInboxAddr, dataLen: 2000, value: 22, author: batcherPriv, good: true},
	...
	for i, tc := range testCases {
		...
		out := DataFromEVMTransactions(DataSourceConfig{cfg.L1Signer(), cfg.Genesis.SystemConfig.BatchInboxAddr, false}, batcherAddr, txs, testlog.Logger(t, log.LevelCrit))
